import * as React from 'react';
import { Link } from 'react-router-dom';

const Button = ({ children, disabled, to, type }) => {
  const base =
    'px-4 inline-block rounded-full bg-yellow-400\n' +
    '             font-semibold uppercase\n' +
    '             tracking-wide text-stone-800 transition-colors\n' +
    '             duration-300 hover:bg-yellow-300 focus:bg-yellow-300 focus:outline-none\n' +
    '             focus:ring focus:ring-yellow-300 focus:ring-offset-2 disabled:cursor-not-allowed';

  const styles = {
    primary: base + 'px-4 py-3 md:px-6 md:py-4',
    small: base + 'px-2 py-2 md:px-5 md:py-2.5 text-sx',
    secondary:
      'px-4 py-3 inline-block border-2 border-stone-300 rounded-full bg-transparent-400\n' +
      '             font-semibold uppercase\n' +
      '             tracking-wide text-stone-400 transition-colors\n' +
      '             duration-300 hover:bg-stone-300 hover:text-stone-800 focus:bg-stone-300 focus:outline-none\n' +
      '             focus:ring focus:ring-stone-300 focus:ring-offset-2 focus:text-stone-800 disabled:cursor-not-allowed',
  };

  if (to)
    return (
      <Link className={styles[type]} to={to}>
        {children}
      </Link>
    );

  return (
    <button disabled={disabled} className={styles[type]}>
      {children}
    </button>
  );
};

export default Button;
